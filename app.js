...
function tryLoadCached(){
  ...
  try{
    ...
    return true;
  }catch(err){ showToast(err.message||"Parse error"); return false; }
}

function renderPlots(){
  els.plots.innerHTML = "";
  const timeIdx = findTimeIndex(headers);
  if (timeIdx === -1){ showToast("No 'Time' column found.", "error"); return; }
  ...
  for (let i=0;i<headers.length;i++){
    ...
    const title = document.createElement("div"); title.className="plot-title"; title.textContent=headers[i];
...
function handleFile(file){
  ...
  r.onerror = () => showToast("Failed to read file.");
  r.onload  = (e)=>{
    try{
      ...
      els.plots.innerHTML=""; showToast(err.message||"Parse error");
    }
  };
  r.readAsText(file);
}
...
["dragenter","dragover"].forEach(ev=>{
  els.dropzone.addEventListener(ev,(e)=>{ e.preventDefault(); e.stopPropagation(); e.currentTarget.classList.add("dragover"); });
});
["dragleave","drop"].forEach(ev=>{
  els.dropzone.addEventListener(ev,(e)=>{ e.preventDefault(); e.stopPropagation(); e.currentTarget.classList.remove("dragover"); });
});
els.dropzone.addEventListener("drop",(e)=>{ const f=e.dataTransfer.files?.[0]; if (f) { lastFile=f; handleFile(f); }});
